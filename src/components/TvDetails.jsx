...
const TvDetails = () => {
  ...
  const { info } = useSelector((state) => state.tv); //tv ko show krne ke liye tv ko save krna padega jiske liye tv ko redux se lana padega
  console.log(info);
  useEffect(() => {
    dispatch(asyncloadtv(id));

    return () => {
      //isse tv remove ho jayegi component se
      dispatch(removetv());
    };
  }, [id]);
  ...
  return info ? (
    <div
      ...
      className="h-[174vh] relative w-screen px-[10%]"
    >
      {/* part 1 navigation */}
      ...
      <div className="flex w-full">
        ...
        <div className="content ml-[5%] text-white">
          ...
          <h1 className="text-xl mb-2 mt-2">tv Translated</h1>
      ...
      <div className="w-[100%] flex overflow-y-hidden mb-5 p-5">
          {info.detail.seasons.length>0? info.detail.seasons.map((s,i) => (
            <div className="w-[15vh] mr-[8%]">
              ...
              <h1 key={i} className="text-xl text-zinc-300 mt-3 font-semibold">
                {s.name  }
              </h1>
            </div>
          )):(
